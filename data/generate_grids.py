...
def generate_grid(size, density, object_density_range, object_separation, goal_center, goal_extents):
    ...
    while grid.sum() / float(np.prod(grid.shape)) < density:
        ...
        added = set()
        fringe = [center]
        while len(added) / float(np.prod(grid.shape)) < object_density and len(fringe) > 0:
            ...
            f_children = [tuple(n + f) for n in neighbors if is_valid_position(size, n + f) and \
                                                             ...
                                                             obstacle_min_dist(grid, n + f, added) >= object_separation*max(size)]
            fringe += f_children
...
if __name__ == '__main__':
    ...
    for i in xrange(args.num_grids):
        grid = generate_grid(descr['size'], descr['density'], descr['object_density_range'], descr['object_separation'],
                             descr['goal']['center'], descr['goal']['extents'])
        save_grid(grid, os.path.join(os.path.dirname(args.yaml), 'grid{0}'.format(i)))
